...
def edit_profile():
    if 'loggedin' in session:
        ...
        if request.method == 'POST':
            ...
            cursor.execute("""
                            UPDATE staffadmin SET first_name = %s, last_name = %s, email = %s, address = %s, phone_number = %s
                            WHERE staff_id = %s AND user_id = %s""", 
                            (first_name, last_name, email, address, phone_number, staff_id, session['user_id']))
        ...
        cursor.execute("""
                        SELECT u.*, s.*
                        FROM user u
                        JOIN staff s a ON u.user_id = s.user_id
                        WHERE s.user_id = %s""", (session['user_id'],))
